...
We apply three complementary security testing methodologies to LLVM's
...
differential fuzzing of 546 randomly generated programs. We find that
LLVM's APX code generation is \emph{correct but brittle}: zero
semantic mismatches across all fuzzing campaigns, but 52\% of
adversarial test functions caused APX pattern failures. We also
...
We compiled 265 source files from five SPEC~CPU~2017 benchmarks
...
with \texttt{+cf}. We disassembled object files with
\texttt{llvm-objdump} and counted APX mnemonics. We also performed a
...
To test the boundaries, we wrote 23 functions combining APX-eligible
...
on Apple Silicon. We designed an \emph{IR-level differential testing}
...
depth~2) and \mbox{\texttt{-{}-no-volatiles}}---our adversarial
...
We compared APX output against non-APX patterns
...
Our IR-level approach tests optimization \emph{decisions} (SimplifyCFG
...
We tested optimization \emph{decisions}, not APX instruction
...
fuzzing. Our work applies differential fuzzing specifically to ISA
...
SIMD/AVX~\cite{taneja2020testing}; to our knowledge, this is the
...
We applied static emission analysis, adversarial context testing, and
